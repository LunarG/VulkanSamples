...
static VkBool32 VKAPI_PTR vkDebugReportCallbackEX_impl(
    ...
    if (flags & VK_DEBUG_REPORT_WARNING_BIT_EXT) {
        __android_log_print(ANDROID_LOG_WARN,"Vulkan Debug Message: ",
                            "%s -- %s", pLayerPrefix, pMessage);
    }
    ...
    if (flags & VK_DEBUG_REPORT_ERROR_BIT_EXT) {
        __android_log_print(ANDROID_LOG_ERROR,"Vulkan Debug Message: ",
                            "%s -- %s", pLayerPrefix, pMessage);
    }
    if (flags & VK_DEBUG_REPORT_DEBUG_BIT_EXT) {
        __android_log_print(ANDROID_LOG_DEBUG,"Vulkan Debug Message: ",
                            "%s -- %s", pLayerPrefix, pMessage);
    }
...
void LayerAndExtensions::CheckLayerLoadingSequence(
    ...
    for (uint32_t idx = 0; idx < size; ++idx) {
        if (!strcmp(layers[idx], kUniqueObjectLayer))
           uniqueObjIdx = idx;
        if (!strcmp(layers[idx], kGoogleThreadingLayer))
            threadingIdx = idx;
    }
    if (uniqueObjIdx != -1) {
...
bool LayerAndExtensions::InitInstExts(void) {
    ...
    CALL_VK(vkEnumerateInstanceExtensionProperties(nullptr,
...
LayerAndExtensions::LayerAndExtensions(void) {
    ...
    if (instLayerCount_) {
        ...
        CALL_VK(vkEnumerateInstanceLayerProperties(&instLayerCount_,
                                                   instLayerProp_));
    }
...
LayerAndExtensions::~LayerAndExtensions() {
    ...
    if (vkCallbackHandle_) {
        vkDestroyDebugReportCallbackEXT(instance_,
                                        vkCallbackHandle_, nullptr);
    }
...
void LayerAndExtensions::InitDevLayersAndExt(VkPhysicalDevice physicalDevice) {
    ...
    if (devLayerCount_ == 1) {
        ...
        if (devLayerProp_)
            delete [] devLayerProp_;
        devLayerProp_ = (instLayerCount_ ?
                        (new VkLayerProperties[instLayerCount_]) : nullptr);
    ...
    if (ext.count) {
        ...
        CALL_VK(vkEnumerateDeviceExtensionProperties(
                physicalDev_, nullptr,&ext.count, ext.prop));
        devExtProp_.push_back(ext);
    }
    for (int i=0; i < devLayerCount_; i++) {
        ext.count = 0;
        CALL_VK(vkEnumerateDeviceExtensionProperties(
                physicalDev_, devLayerProp_[i].layerName,
                &ext.count, nullptr));
        if (ext.count) {
            ext.prop = new VkExtensionProperties[ext.count];
            assert(ext.prop);
...
bool LayerAndExtensions::HookDbgReportExt(VkInstance instance) {
    ...
    CALL_VK(vkCreateDebugReportCallbackEXT(instance, &dbgInfo, nullptr, &vkCallbackHandle_));
    return true;
}
