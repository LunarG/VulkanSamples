...
void Hologram::prepare_images(VkSwapchainKHR swapchain)
{
    ...
    std::vector<const void *> barrier_pointers;
    barriers.reserve(images_.size());
    barrier_pointers.reserve(images_.size());
    for (auto img : images_) {
        barrier.image = img;
        barriers.push_back(barrier);
        barrier_pointers.push_back(&barriers.back());
    }

    vk::BeginCommandBuffer(primary_cmd_, &primary_cmd_begin_info_);
    vk::CmdPipelineBarrier(primary_cmd_,
            VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
            VK_DEPENDENCY_BY_REGION_BIT, barrier_pointers.size(), barrier_pointers.data());
...
void Hologram::update_objects(Worker &worker)
{
    ...
    begin_info.framebuffer = framebuffers_[worker.fb_];
    vk::BeginCommandBuffer(cmd, &begin_info);

    vk::CmdSetViewport(cmd, 1, &viewport_);
    vk::CmdSetScissor(cmd, 1, &scissor_);
