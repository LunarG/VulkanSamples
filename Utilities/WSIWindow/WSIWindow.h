...
class WSIWindow {
  ...
  public:
    ...
    //--Event loop--
    EventType GetEvent(
        bool wait_for_event = false); // Return a single event from the queue (lower-level alternative to using "ProcessEvents")
    bool ProcessEvents(bool wait_for_event =
                           false); // Poll for events, and call appropriate event handlers. Returns false if window is being closed.
