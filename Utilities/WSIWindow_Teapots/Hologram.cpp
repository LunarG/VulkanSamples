...
Hologram::Hologram(const std::vector<std::string> &args)
    : Game("Teapots", args), multithread_(true), use_push_constants_(false),
...
void Hologram::draw_objects(Worker &worker)
{
    ...
    if (!use_push_constants_) {
        ...
        VkDeviceSize ncas = physical_dev_props_.limits.nonCoherentAtomSize;
        if (range.offset % ncas) {
            VkDeviceSize backup = range.offset % ncas;
            range.offset -= backup;
            range.size += backup;
        }
        if (range.size % ncas) {
            range.size += ncas - (range.size % ncas);
        }
