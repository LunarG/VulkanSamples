...
vkCreateDevice(VkPhysicalDevice gpu, const VkDeviceCreateInfo *pCreateInfo,
	...
	my_device_data->pfnQueuePresentKHR = (PFN_vkQueuePresentKHR)pTable->GetDeviceProcAddr(
...
vkQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR *pPresentInfo) {
	...
	if (seconds > 0.5) {
		...
		layer_data *my_instance_data = get_my_data_ptr(get_dispatch_key(my_data->gpu), layer_data_map);
...
vkCreateWin32SurfaceKHR(VkInstance instance, const VkWin32SurfaceCreateInfoKHR* pCreateInfo,
	...
	GetWindowText(my_data->hwnd, my_data->base_title, 1000);
...
#elif defined(VK_USE_PLATFORM_XCB_KHR)
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL
vkCreateXcbSurfaceKHR(VkInstance instance, const VkXcbSurfaceCreateInfoKHR* pCreateInfo,
    const VkAllocationCallbacks* pAllocator, VkSurfaceKHR* pSurface) {
    layer_data *my_data =
        get_my_data_ptr(get_dispatch_key(instance), layer_data_map);
